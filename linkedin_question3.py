...
for line in f:
	...
	time = tmplist[2]
	tmplist = time.split(":")
	newtime=tmplist[0] + ":" + tmplist[1]
	#print(newtime, oldtime)
	# if it is the first time through or newtime is same as previous
	if ((newtime == oldtime) or (count==0)):
		count=count+1
	else:
		print(oldmonth, oldday, oldtime + "," + str(count))
		count=1
	oldtime=newtime
	oldmonth=month
	oldday=day
