...
# AWB NOTES: Refactor to use associative arrays
# will need a two dimensional array
# grrr, multidimensional arrays aren't implemented in bash
declare -A datestring
declare -A messagecount

while read month day mytime
do
  newtime=`echo $mytime |sed -e "s/\([^:]*:[^:]*\):.*/\1/"`
  newdate="$month $day $newtime"
  #echo $newtime
  # the following line is not portable - works on linux but not macOS
  #dateasint=`date -d "$mytime" +%s`
  # macos
  dateasint=`date -j -f "%h %d %H:%M" "$newdate" +%s`
  #echo $dateasint
  # yet another snafu - bash 3 on macos does not support associative arrays
  # brew install
  datestring[${dateasint}]="$newdate"
  if [ -z "${messagecount[$dateasint]}" ]
  then
	#echo initializing message count
  	messagecount[${dateasint}]=1
  else
	#echo updating message count
  	messagecount[${dateasint}]=`expr ${messagecount[$dateasint]} + 1`
  fi
done <messages

echo "minute,number_of_messages"

for key in ${!datestring[@]}
do
    echo "${datestring[$key]},${messagecount[$key]}"
done
