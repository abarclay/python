...
def getProg(mystring):
	prog=re.match("(.*[^:([])",mystring)
	return(prog.group(1))

# one pass through the file
# get program names first
# don't forget to output the add header to output with the dynamic pname list
...
for line in f:
	...
	tmplist = time.split(":")
	newtime=month + day + " " + tmplist[0] + ":" + tmplist[1]
	prog=getProg(program)
	print(program)
f.close()
